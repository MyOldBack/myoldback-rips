...
export default function ItemCardsScreen() {
  ...
  const { data: cards, isLoading, refetch, isRefetching } = useListItemCards(itemId, {
    query: { queryKey: getListItemCardsQueryKey(itemId) },
  });

  const addMutation = useAddItemCard({
    mutation: {
      onSuccess: () => {
        qc.invalidateQueries({ queryKey: getListItemCardsQueryKey(itemId) });
        setShowModal(false);
        setForm(EMPTY_FORM);
      },
    },
  ...
  const deleteMutation = useDeleteItemCard({
    mutation: {
      onSuccess: () => {
        qc.invalidateQueries({ queryKey: getListItemCardsQueryKey(itemId) });
  ...
  const handleAdd = () => {
    ...
    addMutation.mutate({ id: itemId, data: { ...form, imageUrl: null } });
  };
    ...
    <>
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        <View style={[styles.header, { paddingTop: topPad + 8 }]}>
          ...
          <Text style={[styles.title, { color: colors.foreground }]}>Item Cards</Text>
          <TouchableOpacity
                    ...
                    {isConfirming ? (
                      <View style={styles.confirmRow}>
                        ...
                        <TouchableOpacity
                          ...
                          onPress={() => deleteMutation.mutate({ id: itemId, cardId: card.id })}
                          disabled={isDeleting}
                        >
      ...
      <Modal visible={showModal} transparent animationType="slide">
        ...
        >
          <View style={[styles.sheet, { backgroundColor: colors.card }]}>
            <Text style={[styles.sheetTitle, { color: colors.foreground }]}>Add Card</Text>

            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              {([
                { label: "Player Name *", key: "playerName", placeholder: "e.g. Mike Trout" },
